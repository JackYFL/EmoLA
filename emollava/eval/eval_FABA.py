...
def get_args():
    ...
    parser.add_argument("--base-path", type=str, default="./checkpoints/llava-v1.5-7b")
    parser.add_argument("--task-type", type=str, default="AU")
    parser.add_argument("--model-path", type=str, default="./saved_checkpoints/llava-v1.5-7b-AUgpt")
    parser.add_argument("--extra-name", type=str, default=None)
    parser.add_argument("--dataset", type=str, default="FABA", choices=['EmotioNet', 'BP4D', 'DISFA', 'GFT', 'FABA'])
    ...
    parser.add_argument("--output-dir", type=str, default='./output_test')
...
def eval_model(args, query, image_file, landmark_feature_path, model_name, model, image_processor, tokenizer):
    qs = query
    ...
    org_landmark_feature = np.load(landmark_feature_path)
    org_landmark_feature = torch.tensor(org_landmark_feature).half().cuda()
...
def eval():
    ...
    tokenizer, model, image_processor, context_len = load_pretrained_model(args.model_path, args.model_base, model_name, args.model_path)
    ...
    for img_id, img in tqdm.tqdm(enumerate(annos), desc="Processing images", unit="img_id"):
        if not os.path.exists(f'{output_jsons_dir}/{img_id}.json'):    
            img_path, img_label, gpt_output = img['image'], img['label'], img['gpt4']
            img_path = os.path.join(data_dir, img_path)        
            img_landmark_feature_path = os.path.join(data_dir, img['landmark'])
            
            query = message
            emotion_description = eval_model(args, query, img_path, img_landmark_feature_path, model_name, model, image_processor, tokenizer)
            ...
            if pred!=target:
                failed_case = {"image_path": img_path, "given_label": img_label, "prediction": emotion_description, "rouge": f1}
            ...
            output = {"image_path": img_path, "label": emotion_description, "gt": img['label'], "rouge": f1}
            with open(f'{output_jsons_dir}/{img_id}.json', 'w', encoding='utf-8') as f:
                json.dump(output, f, ensure_ascii=False, indent=2)
        else:
            with open(f'{output_jsons_dir}/{img_id}.json') as f:
                results = json.load(f)
            emotion_description = results["label"]
            img_label = results["gt"]
            # rouge = results["rouge"]
            if args.task_type == 'AU':
                pred = AU2Labels(emotion_description, AU2indices)
                target = AUint2Labels(img_label, AU2indices)                
    ...
    if args.task_type == 'emotion':
        ...
        if 'GFT' in args.dataset:
            select_AUs = ['1', '2', '4', '6', '10', '12', '14', '15', '23', '24']
        elif 'EmotioNet' in args.dataset:
